...
class ExecutionRepository:
    ...
    async def get_baseline_execution(
        ...
        """
        Find the first successful execution on the same context.

        This serves as the "no memory" baseline for memory speedup metrics.
        Returns the earliest completed execution with the same context_hash.
        """
        query = (
            select(Execution)
            .where(Execution.context_hash == context_hash)
            .where(Execution.status == ExecutionStatus.COMPLETED)
            .order_by(asc(Execution.started_at))
        )
        if session_id:
            query = query.where(Execution.session_id == session_id)
    ...
    async def save_execution_metrics(
        ...
        """Save computed metrics to an execution record."""
        result = await self.session.execute(
            select(Execution).where(Execution.id == execution_id)
        )
        execution = result.scalar_one_or_none()
        ...
        if execution:
            ...
            if memory_speedup_pct is not None:
                execution.memory_speedup_pct = memory_speedup_pct
            await self.session.flush()
    ...
    async def get_metrics_summary(
        ...
        query = select(Execution).where(Execution.metrics.isnot(None))
        if session_id:
            query = query.where(Execution.session_id == session_id)
